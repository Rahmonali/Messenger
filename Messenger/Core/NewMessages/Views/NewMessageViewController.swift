...
class NewMessageViewController: UIViewController {
    ...
    private func configureUI() {
        ...
        NSLayoutConstraint.activate([
            ...
            tableView.topAnchor.constraint(equalTo: contactsLabel.bottomAnchor),
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        dismiss(animated: true) {
            if let windowScene = UIApplication.shared.connectedScenes.first as? UIWindowScene,
               let rootNavigationController = windowScene.windows.first?.rootViewController as? UINavigationController {
                rootNavigationController.pushViewController(chatViewController, animated: true)
            } else {
                print("Root navigation controller not found.")
            }
        }
    }
}
